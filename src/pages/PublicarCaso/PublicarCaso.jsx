...
const PublicarCaso = () => {
  ...
  useEffect(() => {
    const processPendingCaseAfterGoogleAuth = async () => {
      const pendingCaseStr = sessionStorage.getItem('lp_pending_case')
      ...
      try {
        ...
        const fullName = user.user_metadata?.full_name || user.user_metadata?.name || `${user.user_metadata?.given_name || ''} ${user.user_metadata?.family_name || ''}`.trim() || 'Usuario Google'
        const [firstName, ...lastNameParts] = fullName.split(' ')
        const lastName = lastNameParts.join(' ')

        // 1. Guardar/actualizar información clave en client_profiles (Nombre completo y Correo)
        ...
        setCaseText(pendingCase.caseText || '')
        setRegion(pendingCase.region || '')
        setCity(pendingCase.city || '')

        // 2. Guardar el caso en la base de datos
        const { data: caseData, error: caseError } = await supabase
          .from('cases')
          .insert({
            user_id: user.id,
            description: pendingCase.caseText,
            client_email: user.email,
            region: pendingCase.region,
            city: pendingCase.city,
        ...
        sessionStorage.removeItem('lp_pending_case')
        setConfirmedEmail(user.email)
        window.scrollTo({ top: 0, behavior: 'smooth' })
        setStep(3)
      } catch (err) {
        console.error('Error procesando caso pendiente tras Google Auth:', err)
        setSubmitError('Error al guardar el caso tras iniciar sesión con Google: ' + err.message)
      }
    }

    processPendingCaseAfterGoogleAuth()
  }, [])
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      ...
      // 4. Vincular la cuenta anónima con email/contraseña
  ...
  const handleGoogleRegister = async () => {
    ...
    try {
      ...
      if (user && !user.is_anonymous) {
        ...
        await uploadAndSaveCase(user.id, user.email)
        return
      }

      sessionStorage.setItem('lp_pending_case', JSON.stringify({
        caseText,
        region,
        city
      }))
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/publicar-caso`,
          data: { role: 'client' }
        }
      })
      ...
      console.error('Error al continuar con Google:', error)
      setSubmitError(error.message || 'Error al conectar con Google.')
    } finally {
      setRegisterLoading(false)
    }
  }
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex justify-center">
                    <button 
                      ...
                      className="social-btn w-full flex items-center justify-center gap-3 py-3.5 px-6 bg-white border-[1.5px] border-slate-200 rounded-2xl font-semibold text-[15px] text-on-background hover:bg-slate-50 disabled:opacity-50"
                    >
